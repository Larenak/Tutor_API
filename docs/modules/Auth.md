...
> Версия: 1.2
> Статус: Базовый контур реализован
...
- `POST /api/v1/auth/register` — атомарно создаёт пользователя, Argon2id-хеш
  пароля и первую refresh-сессию;
- `POST /api/v1/auth/login` — проверяет учётные данные и статус пользователя;
- `POST /api/v1/auth/refresh` — одноразово ротирует refresh-токен под блокировкой
  строки сессии;
- `POST /api/v1/auth/logout` — отзывает refresh-сессию текущего пользователя;
- `GET /api/v1/users/me` — возвращает профиль по access-токену;
- административные API требуют роль `admin`.

Access-токены не сохраняются в базе. Refresh-токены хранятся только в виде
SHA-256-хеша. Каждый JWT содержит уникальный `jti`, время выпуска и срок действия.
В браузере оба токена живут в `sessionStorage`, поэтому не переносятся между
вкладками и удаляются при закрытии вкладки или выходе.
